...
class RecursiveAbstractiveProcessing4TreeOrganizedRetrieval:
    ...
    def __call__(self, chunks, random_state, callback=None):
        ...
        def summarize(ck_idx, lock):
            ...
            try:
                ...
                with lock:
                    ...
                    logging.info(f"【Lynn1-Debug】RAPTOR SUM append! ck_idx={ck_idx} len(chunks)={len(chunks)}")
        ...
        while end - start > 1:
            ...
            logging.info(f"【Lynn1-Debug】len(chunks)={len(chunks)} end={end} start={start} n_clusters={n_clusters}")
            labels.extend(lbls)
            layers.append((end, len(chunks)))
            if callback:
                callback(msg="Cluster one layer: {} -> {}".format(end - start, len(chunks) - end))
